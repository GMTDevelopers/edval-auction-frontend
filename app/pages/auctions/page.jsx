                    ...
                    {loading? 
                      ...
                      <div className="row3">
                        {auctionData?.length > 0 && auctionData?.map((data)=>(
                          <AuctionCard key={data.id} auctionId={data.id} name={data.name} price={data.commitment_fee} img={data?.cover_image_url || `/images/homepage/auction1.png`} duration={data.duration_minutes} time={data.scheduled_at} auctStatus={data.status} slug={data.slug}/>
                        ))}
                        {auctionData?.length===0 && defaultAuctionData?.map((data)=>(
                          <AuctionCard key={data.id} auctionId={data.id} name={data.name} price={data.commitment_fee} img={data?.cover_image_url} time={data.time} status={data.status}/>
                        ))}
                    </div>}
                </div>
            </div>
