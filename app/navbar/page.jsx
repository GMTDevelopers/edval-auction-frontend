        ...
        <div className={styles.navbar}>
            <div className="container">
                <div className={styles.navContainer}>
                    ...
                    {isAuthenticated && <div onClick={() => !isOpen ? setIsOpen(true) : setIsOpen(false)} className={`btn ${styles.navBtnLoggedIn}`}>
                ...
                <div className={isOpen&&!isAuthenticated ? `${styles.menu}` : `${styles.noMenu}`}>
                    <li className='btn' onClick={()=> openModal(<Tab />)}>Sign in to my account</li>
                    <li className='btn' onClick={()=> openModal(<Tab />)}>Create user account</li>
                    <li className='btn'>Create artist account</li>
                </div>
                <div className={isOpen&&isAuthenticated ? `${styles.menu}` : `${styles.noMenu}`}>
                    <li className='btn'><Link href="/user/client">My Orders</Link></li>
                    <li className='btn'>My Account</li>
                    <li className='btn' onClick={() => logout()} style={{color:"#FB0000"}}>Sign out</li>
                </div>
            </div>
        </div>
    );
